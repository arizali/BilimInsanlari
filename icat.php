...
$q = dbquery("select * from icatlar as i
left join biyografiler as b on i.icatSahip = b.ID where icatID='$id' and icatSef='$sef'");

if(dbrows($q)!=1) { redirect(SURL."404"); exit(); }
$haber_bilgisi = dbarray($q);
dbquery("update icatlar set icatHit = (icatHit+1) where icatID='$id' limit 1");
$sayfa_ozellikleri["baslik"] = "Bilim İnsanları » ".$haber_bilgisi["AdSoyad"]." » İcatlar ve Keşifler » ".$haber_bilgisi["icatBaslik"];
$sayfa_ozellikleri["baslik2"] = $haber_bilgisi["AdSoyad"]." » ".$haber_bilgisi["icatBaslik"];
$sayfa_ozellikleri["facebook"] = true;
$sayfa_ozellikleri["url"] = LinkOlustur("icat", array("id"=>$id, "sef"=>$sef));
$sayfa_ozellikleri["resim"] = SURL."resimler/icat/310b/".$haber_bilgisi["icatResim"];
$sayfa_ozellikleri["aciklama"] = $haber_bilgisi["icatKisabilgi"];
$yorum_hata = false;
...
if(!$yorum_hata) {
dbquery("insert into yorumlar values ('','$id','0','i','$yorum_ad','$yorum_mail','$yorum_web','$yorum_metin','".time()."','aktif','0','0')");	
redirect(LinkOlustur("icat", array("id"=>$id, "sef"=>$sef)));
exit();
}
}
RenderHead(); 
...
<div class="genisbaslik"><h2 class="baslikbu" style="font-size:15px;"><?=$haber_bilgisi["icatBaslik"];?></h2></div>
<div class="ayrac"></div>
<div class="icerikalani">
<div class="descbox">
<div style=" font-size:10px; padding:5px; background:#eee;">Eklenme Tarihi: <strong><?=date("d.m.Y h:s", $haber_bilgisi["icatZaman"]);?></strong><br />
Okunma: <strong><?=$haber_bilgisi["icatHit"]+1;?></strong></div>
<div class="ayrac"></div>
<?php 
	$ozellik=array();
	$ozellik["AdSoyad"] = $haber_bilgisi["AdSoyad"];
	$ozellik["Resim"] = $haber_bilgisi["Resim"];
	$ozellik["DTarih"] = $haber_bilgisi["Dogum"];
	$ozellik["OTarih"] = $haber_bilgisi["Olum"];
	$ozellik["Sehir"] = $haber_bilgisi["Ulke"];
	$ozellik["Link"] = LinkOlustur("biyografi", array("sef"=>$haber_bilgisi["AdSef"]));
	BiyoBox($ozellik);
?>  
<div class="cl"></div>  
<div class="ayrac"></div>
<img class="ccres"  src="<?=SURL."resimler/icat/310b/".$haber_bilgisi["icatResim"];?>" alt="<?=$haber_bilgisi["icatBaslik"];?>" />
...
echo preg_replace('#<p[^>]*>(\s|&nbsp;?)*</p>#', '', stripslashes($haber_bilgisi["icatMetin"]));?>
<div class="cl"></div>
...
<div class="ayrac"></div>
<?php 
YorumGoster($haber_bilgisi["ID"], "icat");
?>
